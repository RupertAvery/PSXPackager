...
namespace Popstation
{
    public abstract class PbpWriter
    {
        ...
        private void EnsureRequiredResourcesExist()
        {
            ...
            using (var basePbp = new FileStream(convertInfo.BasePbp, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                ...
                if (!convertInfo.Icon0.Exists)
                {
                    ...
                    convertInfo.DataPsp = new Resource(ResourceType.PSP, icon0_buffer, icon0_size);
                }
        ...
        protected CueFile GetDummyCueFile()
        {
            return new CueFile()
            {
                FileEntries =
        ...
        protected byte[] GetTOCData(CueFile cue, uint isosize)
        {
        ...
        protected uint WriteDisc(DiscInfo disc, uint psarOffset, Stream outputStream, int ciso, CancellationToken cancellationToken)
        {
            uint iso_position;
            ...
            using (var _in = new FileStream(disc.SourceIso, FileMode.Open, FileAccess.Read))
            {
                ...
                iso_position = (uint)outputStream.Position - psarOffset;
                ...
                else
                {
                    ...
                    while ((bytesRead = (uint)_in.Read(buffer2, 0, BLOCK_SIZE)) > 0)
                    {
                        //totSize += bytesRead;
                        //curSize += bytesRead;
